...
- **macOS / Apple Silicon is not a real test host** — The tree is **x86-64-only** (`immintrin.h`, haswell march). Native arm64 builds fail; `linux/amd64` under Docker Desktop/Rosetta is also non-authoritative (mass SIGILL, io_uring/hugetlbfs aborts, no hugepages → event-ring ENOSPC). **Agents: do not run full `ctest` or multi-hour docker rebuilds on a Mac** — they burn time/tokens and the failures are environmental, not merge bugs. Prefer: compile only the TUs you touched (or a short targeted `-R` filter if a Linux amd64 box is available), treat CI / a real Linux x86-64 runner as the gate, and otherwise assume tests pass after a careful merge audit. Rust `cargo test` for event crates also needs Linux (macOS link fails on missing `monad_event_os_compat`).
